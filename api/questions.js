...
module.exports = async function handler(req, res) {
  ...
  if (!LEVELS.includes(level)) {
    return json(res, 400, { ok: false, message: "level tidak dikenal" });
  }

  try {
    // Prefer Blob override (uploaded via admin)
    ...
    const fromDisk = readStaticQuestion(level);
    if (fromDisk) {
    ...
    return json(res, 404, { ok: false, message: `Soal ${level} tidak ditemukan` });
  } catch (e) {
